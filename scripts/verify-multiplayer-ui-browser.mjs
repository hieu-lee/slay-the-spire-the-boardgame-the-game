...
try {
  for (const viewport of [{ name: 'desktop', width: 1440, height: 900 }, { name: 'landscape-phone', width: 560, height: 315 }]) {
    ...
    await lobby.waitFor()
    await page.waitForFunction(() => document.querySelector('.online-lobby__start')?.disabled === false)
    ...
    const startButton = lobby.getByRole('button', { name: 'Enter the Spire', exact: true })
    assert.equal(await startButton.isEnabled(), true, `${viewport.name}: Start was not ready before lobby edits`)
    const startFilter = await startButton.evaluate((button) => getComputedStyle(button).filter)
    ...
    let releaseAscension
    const ascensionGate = new Promise((resolve) => { releaseAscension = resolve })
    await page.route('**/api/rooms/*/ascension', async (route) => { await ascensionGate; await route.continue() })
    const increase = lobby.getByRole('button', { name: 'Increase Ascension' })
    await increase.click()
    await page.waitForFunction(() => document.querySelector('.online-lobby__start')?.getAttribute('aria-busy') === 'true')
    assert.equal(await startButton.isDisabled(), true, `${viewport.name}: pending Ascension change permits starting the run`)
    assert.equal(await startButton.evaluate((button) => getComputedStyle(button).filter), startFilter,
      `${viewport.name}: pending Ascension change flashes the Start button`)
    await page.screenshot({ path: join(out, `${viewport.name}-lobby-pending-ascension.png`) })
    await increase.click()
    releaseAscension()
    ...
    await page.route('**/api/rooms/*/character', async (route) => { await characterGate; await route.continue() })
    await lobby.locator('.online-character-roster').getByRole('button', { name: 'Silent', exact: true }).click()
    await page.waitForFunction(() => document.querySelector('.online-lobby__start')?.getAttribute('aria-busy') === 'true')
    assert.equal(await startButton.isDisabled(), true, `${viewport.name}: pending character change permits starting the run`)
    assert.equal(await startButton.evaluate((button) => getComputedStyle(button).filter), startFilter,
      `${viewport.name}: pending character change flashes the Start button`)
    await page.screenshot({ path: join(out, `${viewport.name}-lobby-pending-character.png`) })
    releaseCharacter()
    await page.waitForFunction(() => document.querySelector('.online-character-roster__portrait[aria-pressed="true"]')?.getAttribute('aria-label') === 'Silent')
    await page.unroute('**/api/rooms/*/character')
    await lobby.locator('.online-lobby__settings > summary').click()
