...
for (const activeState of ['active', 'inactive']) {
  ...
  try {
    ...
    mkdirSync(join(release, 'node_modules/@openai/codex-sdk/dist'), { recursive: true })
    ...
    writeFileSync(join(release, 'node_modules/@openai/codex-sdk/dist/index.js'), '')
...
assert.match(pages, /VITE_ASSET_CDN_ORIGIN="https:\/\/cdn\.jsdelivr\.net\/gh\/\$GITHUB_REPOSITORY@\$DEPLOY_SHA\/public\/assets" VITE_CAMPFIRE_BACKUP_ORIGIN="https:\/\/raw\.githubusercontent\.com\/\$GITHUB_REPOSITORY\/\$DEPLOY_SHA\/public\/assets" VITE_SINGLE_PLAYER=false VITE_HOSTED_SESSION=true pnpm build/)
...
assert.match(pages, /find dist\/assets\/noncombat\/campfire -type f -name '\*_firecamp\.webp' ! -name 'empty_firecamp\.webp' -delete/)
assert.match(pages, /for file in empty_firecamp\.webp rest\.webp smith\.webp; do test -s/)
...
assert.match(pages, /if ! curl[\s\S]*cdn\.jsdelivr\.net[\s\S]*ironclad_silent_firecamp\.webp[\s\S]*content-type: image\/webp'; then/)
assert.match(pages, /raw\.githubusercontent\.com[\s\S]*ironclad_silent_firecamp\.webp[\s\S]*content-type: image\/webp'[\s\S]*fi/)
assert.match(pages, /du -sb dist[\s\S]*-le 850000000/)
