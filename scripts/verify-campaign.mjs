...
check('Quick Start and Catch Up award marks only for Bosses each character could fight', () => {
  const quick = finishCampaign(createCampaignProgress(), {
    runId: 'quick-act-three', characters: ['ironclad'], bossesDefeated: 1,
    highestBossActDefeated: 3, startedAtAct: 3, ascensionPlayed: 0,
  })
  assertEqual(quick.characters.ironclad, 2)

  const catchUp = finishCampaign(createCampaignProgress(), {
    runId: 'catch-up-act-two', characters: ['ironclad', 'silent'], bossesDefeated: 2,
    highestBossActDefeated: 2, joinedAfterBosses: { silent: 1 }, ascensionPlayed: 0,
  })
  assertEqual(catchUp.characters.ironclad, 3)
  assertEqual(catchUp.characters.silent, 2)
})
