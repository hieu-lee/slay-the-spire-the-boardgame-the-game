...
function readThrough(thread, reader, now) {
  const from = reader === 'player' ? 'developer' : 'player'
  return thread.letters.reduce((latest, letter) => letter.from === from ? Math.max(latest, letter.at) : latest, now)
}

/** Player letters that have not been handled by either delegated mailbox account. */
export const developerUnread = (mail) => mail.reduce((total, thread) => total + unreadFor(thread, 'developer'), 0)
...
export function playerInbox(mail, profile, { markRead = false, now = Date.now() } = {}) {
  ...
  if (markRead && thread && unread) thread.playerReadAt = readThrough(thread, 'player', now)
...
export function developerInbox(mail, profiles, { username: name, markRead = false, now = Date.now() } = {}) {
  ...
  const threads = username === undefined
    ? mail.filter((thread) => thread.letters.some((letter) => letter.from === 'player'))
    : mail.filter((thread) => thread.username === username)
  let changed = false
  const summary = threads.map((thread) => {
    const unread = unreadFor(thread, 'developer')
    if (markRead && unread) {
      thread.developerReadAt = readThrough(thread, 'developer', now)
      changed = true
    }
    return {
      username: thread.username,
  ...
  return { threads: summary.sort((left, right) => right.lastAt - left.lastAt), unread: developerUnread(mail), changed }
}
