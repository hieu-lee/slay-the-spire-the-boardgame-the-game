...
const localMeta = page.locator('.start-menu__run-options')
...
const compactMetaFrame = await page.evaluate(() => {
  const options = document.querySelector('.start-menu__run-options')?.getBoundingClientRect()
  const panel = document.querySelector('.start-menu__meta-panel')?.getBoundingClientRect()
  return {
    documentOverflow: document.documentElement.scrollWidth > innerWidth,
    optionsContained: Boolean(options && options.left >= 0 && options.right <= innerWidth),
...
const arrivalImageHints = await page.locator('.merchant-arrival__party img')
...
await page.getByRole('button', { name: 'Enter merchant shop' }).hover()
const reducedMerchantLift = await page.locator('.merchant-arrival__merchant img')
...
check('Merchant map entry opens the arrival scene with the whole party', () => {
  ...
  assert(arrivalImageHints, 'standing character art is missing eager loading or async decoding')
  ...
  assertEqual(reducedMerchantLift, 'none', 'the Merchant still lifts under reduced motion')
...
const clericBaseRun = postNeowRun(8803, onlineSeats.map(({ playerId: id, name, character }) => ({ id, name, character })))
liveRoom.run = enterRoom(clericBaseRun, roomChoices(clericBaseRun)[0].id)
