...
export async function fetchMailDesk(username?: string, markRead = false): Promise<MailDesk> {
  return desk(await post('/api/mail/desk', { token: profile().token, ...(username ? { username } : {}), markRead }))
}
