...
export function CourierPanel({ players, viewerId, ascension, usedBy, offer, pledge, online = false, onReveal, onResolve }: {
  ...
  const ownsCourier = viewer.relics.some((relic) => relic.defId === 'the_courier');
  ...
  const contribution = online ? Math.min(Math.max(0, viewer.gold - mine), remaining) : players.reduce((sum, player) => sum + player.gold, 0) >= remaining ? remaining : 0;
  const partyCanAfford = players.reduce((sum, player) => sum + player.gold, 0) >= remaining;
  const payments = online ? { [viewer.id]: mine + contribution } : (() => {
    let left = remaining;
    return Object.fromEntries(players.map((player): [string, number] => {
  ...
  const canFund = !potionBlocked && cost !== null && (pledge ? contribution > 0 : (!online || viewer.id === owner.id) && partyCanAfford) && (!online || viewer.id === owner.id || Boolean(pledge)) && (!potionFull || Boolean(pledge?.discardPotionId ?? discardPotionId));
    ...
    >
      ...
      <div className="merchant-board">
        ...
        <div className="merchant-shelf" aria-label="Potions">
          ...
          {!buyerHasSozu && buyer.potions.length >= (ascension >= 4 ? 2 : 3) && buyer.id === player.id ? <label className="merchant-potion-discard">Replace potion<select value={validDiscardPotionId} onChange={(event) => setDiscardPotionId(event.target.value)}><option value="">Choose one</option>{buyer.potions.map((id, index) => <option key={`${id}-${index}`} value={id}>{potionDef(id).name}</option>)}</select></label> : null}
        </div>
        ...
        <div className="event-selectors">
          ...
          {needsRelic && player.relics.length ? (
            <label>
              ...
              <select
                value={relicId}
                onChange={(event) => { setRelicId(event.target.value); if (event.target.value) setPotionIndexes([]); }}
              >
                <option value="">None</option>
          ...
          {needsPotion && player.potions.length ? (
            <fieldset className="event-cards"><legend>Your potions</legend>
              {player.potions.map((id, index) => <button type="button" aria-pressed={potionIndexes.includes(index)} key={`${id}-${index}`} onClick={() => { setRelicId(""); setPotionIndexes((current) => current.includes(index) ? current.filter((value) => value !== index) : maximumPotions === 1 ? [index] : [...current, index].slice(-maximumPotions)); }}>{potionDef(id).name}</button>)}
            </fieldset>
          ) : null}
      ...
      <div className="event-options" ref={eventOptions}>
        ...
        {room.card.id === "knowing_skull" ? (
          <button
            ...
            onClick={() => submit(selectedOptions)}
          >
