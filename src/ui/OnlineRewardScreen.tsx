          ...
          return (
            <div className="reward-screen__player" key={player.id}>
              ...
              {offer.relic === null ? mine ? <p><button type="button" onClick={() => onAction({ kind: 'relicReward', choice: 'reveal' })}>Reveal Relic</button>{' '}
                ...
                : typeof offer.relic === 'string' ? <div className="reward-screen__relic"><strong>{relicDef(offer.relic).name}</strong><ItemImage kind="relic" id={offer.relic} card /><span className="room-item-text">{relicDef(offer.relic).text}</span>
              ...
              {Array.isArray(offer.bossRelics) ? <div className="reward-screen__relic"><strong>Choose a boss Relic</strong>
                {offer.bossRelics.map((id) => <button disabled={!mine} type="button" key={id} onClick={() => onAction({ kind: 'bossRelicReward', choice: 'gain', relicId: id })}><ItemImage kind="relic" id={id} card />{relicDef(id).name} — {relicDef(id).text}</button>)}
              ...
              {offer.potion === null ? mine ? <p>
                ...
                {mine ? <>
                  ...
                  {player.potions.map((held, index) => <button type="button" key={`${held}-${index}`}
                    ...
                    onClick={() => onAction({ kind: 'potionReward', choice: 'replace', potionId: held })}><ItemImage kind="potion" id={held} card />Replace {potionDef(held).name}</button>)}
