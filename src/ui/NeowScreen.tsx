...
type Props = {
  ...
  onReveal: (playerId: string, stage: 'red' | 'reward') => void
...
export function NeowScreen({ players, progress, viewerId, potionLimit, enabled = true, disabledMessage, onViewer, onGold, onReveal, onReward, onEffect, onChoose }: Props) {
  ...
  const unrevealedStage = !activeProgress.redGoldPending && activeProgress.redRewardPending && !activeProgress.redReward ? 'red'
  ...
  return <section className="neow-screen" aria-labelledby="neow-title">
    <header className="neow-screen__header">
      ...
      <span className="neow-screen__progress" role="status">{Object.values(progress).filter((seat) => seat?.done).length}/{players.length} ready</span>
    </header>
    ...
    <div className="neow-faces" aria-label="Dealt Neow cards">
      {players.map((player) => {
        ...
        return <article key={player.id} className={`neow-face${player.id === viewer.id ? ' neow-face--active' : ''}${state?.done ? ' neow-face--done' : ''}`}>
          ...
          {onViewer && player.id !== viewer.id && !state?.done ? <button type="button" onClick={() => onViewer(player.id)}>Resolve {player.name}</button> : null}
        </article>
      })}
    </div>

    <section className="neow-action" aria-labelledby="neow-action-title">
      ...
      {unrevealedStage ? <div className="neow-unrevealed">
        ...
        <div className="neow-offer__actions">
          <button type="button" disabled={!enabled} onClick={() => onReveal(viewer.id, unrevealedStage)}>Reveal {unrevealedKind}</button>
