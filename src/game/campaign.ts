...
export function parseCampaignProgress(value: unknown, fallback = createCampaignProgress()): CampaignProgress {
  const safeFallback = (): CampaignProgress => ({ ...fallback, characters: { ...fallback.characters }, finishedRunIds: [...fallback.finishedRunIds] })
...
export const isActIVUnlocked = (progress: CampaignProgress): boolean => progress.actIV >= ACT_IV_UNLOCK_BOXES
...
export function finishCampaign(progress: CampaignProgress, finish: CampaignFinish): CampaignProgress {
  ...
  if (finish.bossesDefeated < finish.highestBossActDefeated || finish.bossesDefeated > finish.highestBossActDefeated + bonusBosses) throw new Error('highest defeated Boss Act does not match the Boss count')
  ...
  let next = progress
  for (const character of finish.characters) next = awardCampaignMarks(next, character, finish.bossesDefeated + 1).progress
...
export function canEnterActIV(progress: CampaignProgress, keys: SpireKeys, completedAct: number): boolean {
