...
- `title-flame.webp`: seven actual flame shapes extracted from the user-supplied 14.51.42 recording, cropped at `(731, 305, 781, 381)` into a 350×76 transparent strip. Source frames 30, 128, 152, 176, 200, 224, and 248 at 60 fps exclude the later scene transition. The changing silhouettes occur every 400 ms in the 0.25× recording: CSS steps advance every 100 ms in a 700 ms loop. Only the strip translates inside a fixed clipped window; the white base stays steady. Both OS and in-game Reduce Motion settings show the first frame.
